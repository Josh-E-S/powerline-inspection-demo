...
def main():
    ...
    table = {
        ...
        "precision": [round(float(np.interp(t, px, p_macro)), 4)
                      for t in thresholds],
        "recall": [round(float(np.interp(t, px, r_macro)), 4)
                   for t in thresholds],
    }
    OUT.write_text(json.dumps(table, indent=2) + "\n")
    print(f"Wrote {OUT}")
    for t, p, rr in zip(table["thresholds"], table["precision"],
                        table["recall"]):
